...
Usage:
    python GlobalPlans_plotter.py                # all routes
    python GlobalPlans_plotter.py 0 1 8          # only routes 0, 1, 8
    python GlobalPlans_plotter.py --dim 120 2 3  # custom DIM, routes 2 and 3
...
def build_routes(dim):
    ...
    return {
        ...
        1: [g(dim / 3 + dim / 10, dim / 3 + dim / 10), g(dim / 2, dim / 3 + dim / 4.5 - dim / 30),
        ...
        3: [g(2 * dim / 3 - dim / 11, dim / 3 + dim / 10), g(2 * dim / 3 - dim / 4.5 + dim / 25, dim / 2),
        ...
        5: [g(2 * dim / 3 - dim / 10, 2 * dim / 3 - dim / 11), g(dim / 2, 2 * dim / 3 - dim / 4.5 + dim / 30),
        ...
        7: [g(dim / 3 + dim / 11, 2 * dim / 3 - dim / 10), g(dim / 3 + dim / 4.5 - dim / 30, dim / 2),
...
def path_intersections(pts_a, pts_b):
    ...
    for i in range(len(pts_a) - 1):
        ...
        for j in range(len(pts_b) - 1):
            ...
            ang_a = math.degrees(math.atan2(a1[1]-a0[1], a1[0]-a0[0]))
            ang_b = math.degrees(math.atan2(b1[1]-b0[1], b1[0]-b0[0]))
            d = abs((ang_a - ang_b + 180) % 360 - 180)
            incidence = min(d, 180 - d)
            out.append({
                "point": (x, y),
                "dist_a": cum_a[i] + t * seg_a_len,
                "dist_b": cum_b[j] + u * seg_b_len,
                "incidence": incidence,
            })
    return out
